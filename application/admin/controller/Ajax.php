...
class Ajax extends Backend
{
    ...
    public function upload()
    {
        ...
        // 这里改动用于私有文件的上传 by easypay
        $filePath = ROOT_PATH . '/public' . $uploadDir;
        if($this->request->param('private',0) == 1){
            $filePath = ROOT_PATH  . $uploadDir;
        }

        $splInfo = $file->validate(['size' => $size])->move($filePath, $fileName);
        if ($splInfo) {
    ...
    public function apiaccount(){
        ...
        $accountList = ApiAccount::field('id as value,id,name')->select();
        $this->success('', null, $accountList);
    }

    public function apitype()
    {
        $account_id = $this->request->get('account_id/d');

        $channelList = ApiChannel::where([
            'api_account_id'=>$account_id,
            'status'=>'1'
        ])->field('api_type_id')->select();

        if(count($channelList) > 0 ){
            $channelList = collection($channelList)->toArray();
            $typeIds = array_column($channelList,'api_type_id');
            $typeList = ApiType::field('code as `value`,name')->where('id','in',$typeIds)->select();

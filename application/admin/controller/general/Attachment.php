...
class Attachment extends Backend
{
    ...
    public function view()
    {
        ...
        $rules = [
            'id|附件编号' => 'require|integer'
        ];
        $result = $this->validate($data, $rules);
        if (true !== $result) {
            $this->error($result);
        }
        $attachment = \app\common\model\Attachment::get($data['id']);
        if (is_null($attachment)) {
            $this->error('附件不存在！');
        }
        $file_path = ROOT_PATH . $attachment->url;
        if (is_file($file_path)) {
            $file_obj = new File($file_path);
            $file_content = file_get_contents($file_path);
            return \response($file_content, '200', ['Content-Length' => strlen($file_content)])->contentType($file_obj->getMime());
        } else {

...
function remove_css($value){
    static  $obj = null;
    if(is_null($obj)){
        \think\Loader::import('HTMLPurifier.HTMLPurifier',EXTEND_PATH,'.auto.php');
...
function redisLocker(){
    static $obj = null;
    if(is_null($obj)){
        $obj = new \fast\RedisLock(config('redis.lock_server'));
    }
    return $obj;
}
 ...
 * @return bool
 */
function verifyApiSign($data,$md5Key,$pubKey){
    ksort($data);
    reset($data);
    $arg = '';
    foreach ($data as $key => $val) {
 ...
 * @return bool
 */
function checkRepayTime(){

    $now = \date('Hi') - 0;

    $start = str_replace(':','',config('site.txpaytimestart')) - 0;

    $end = str_replace(':','',config('site.txpaytimeend')) - 0;
 ...
 * @return bool
 */
function checkApiDomain($domain,$gateway){

    $urlInfo = parse_url($gateway);

    $gateway = $urlInfo['scheme'].'://'.$urlInfo['host'];

    return strcasecmp($domain, $gateway) == 0;
